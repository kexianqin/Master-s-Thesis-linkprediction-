...
def train_test_split(fr,n_folds=2,edges ='cr'):
    ...
    i = 0
    random.seed(3) #现在为了固定随机数，后面测试时候可以去掉种子
    while i < edges and line != [''] :
        #print(line)
        nodepair_set[random.randint(0,n_folds-1)].append([line[0],line[1]])
        line = fr.readline().strip('\n').split('\t')
        i += 1
...
def just_read_nodepair (fr):
    nodepair_set = []
    line = fr.readline().strip('\n').split('\t')
    while line != ['']:
        # print(line)
        nodepair_set.append([line[0], line[1]])
        line = fr.readline().strip('\n').split('\t')
    fr.close()
    return nodepair_set


def create_vertex(nodepair_set):  #------关于参数，是某一个n_folds,例如nodepair_set[0]
    '''
